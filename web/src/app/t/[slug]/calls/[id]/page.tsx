...
export default async function TeamCallDetailPage({
  params,
}: {
  ...
  const sql = getDb();
  const rows = await sql`SELECT * FROM scorecards WHERE id = ${id} AND team_id = ${team.id} LIMIT 1`;
  ...
  return (
    <div className="report">
      ...
      <nav className="rpt-nav">
        <Link href={`/t/${slug}`} className="nav-brand">Killer Calls</Link>
        <span className="nav-sep">/</span>
        <span className="nav-crumb">{row.rep_name}</span>
        <span className="nav-sep">/</span>
        <span className="nav-crumb">{row.company_name}</span>
        <Link href={`/t/${slug}/settings`} className="nav-back" title="Settings">&#9881;</Link>
        <Link href={`/t/${slug}`} className="nav-back">&larr; Library</Link>
      </nav>
        ...
        <div className="hero-grid">
          <div className="hero-info">
            ...
            <div className="hero-details">
              <Link href={`/t/${slug}/reps/${encodeURIComponent(row.rep_name)}`} className="hero-name">
                {row.rep_name}
              </Link>
        ...
        {sc.close && sc.close.style !== "none" ? (
          ...
          <div className="fw-card fw-card--r" style={{ textAlign: "center", padding: "2rem" }}>
            ...
            <p className="fw-feedback" style={{ marginTop: "0.75rem" }}>
              This rep did not ask for the business. The close is the close phase at 16 points. Every demo should end with a clear commitment ask.
            </p>
          </div>
        )}
